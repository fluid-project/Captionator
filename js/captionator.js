		...
		"captionify": function(element,defaultLanguage,options) {
			...
			if (!objectsCreated) {
				...
				captionator.TextTrack = function TextTrack(id,kind,label,language,trackSource,isDefault) {
				    ...
				    this.processCaptions = function (capTxt) {
					    var TrackProcessingOptions = this.videoNode._captionatorOptions || {};
						...
						var captionData = captionator.parseCaptions(capTxt,TrackProcessingOptions);
						this.readyState = captionator.TextTrack.LOADED;
						this.cues.loadCues(captionData);
						this.activeCues.refreshCues.apply(this.activeCues);
						this.videoNode._captionator_dirtyBit = true;
						captionator.rebuildCaptions(this.videoNode);
						    ...
						    ajaxObject.onreadystatechange = function (eventData) {
							    if (ajaxObject.readyState === 4) {
								    if(ajaxObject.status === 200) {
								        currentTrackElement.processCaptions(ajaxObject.responseText);
								    } else {
